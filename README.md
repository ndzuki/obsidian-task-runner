...
| `otg validate-doc <task>` | 检查任务 frontmatter 是否合法 |
| `otg repair-doc <task>` | 修复损坏的 frontmatter（保留多行字段和列表） |
...
6. **任务 frontmatter 损坏（"parse error"）**：运行 `otg validate-doc <task>` 诊断，`otg repair-doc <task>` 修复（保留多行字段和列表，丢弃孤立的非 YAML 文本）。修复后 `validate-doc` 应输出 `frontmatter OK`。
7. **需要重新安装 Skill**：先执行 `otg install --dry-run`，确认路径无误后再执行 `otg install --force`。
