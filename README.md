...
| 方式 | 命令 | 说明 |
|------|------|------|
| 实时日志 | `tail -f ~/.claude/logs/task-runner.log` | Claude 每一步输出 |
| 任务文件 | 在 Obsidian 中打开对应 Task | `## 实现记录` 逐步填充 |
| systemd | `journalctl --user -fu claude-task-runner` | 服务层日志 |
...
install.sh                         # 一键安装/卸载
claude-task-runner.service         # systemd 兜底轮询
claude-task-runner.timer           # systemd 定时器
claude-task-watcher.service        # systemd 事件触发
```
...
| `switch_settings` | bool | ⛔ 已弃用，改由 `assignee` 字段选择 agent（`codex`/`claude`/`claude+human`） |
| `assignee` | string | 委派执行 agent：`codex`（Codex CLI）、`claude`（Claude Code）、`claude+human` |
...
systemctl --user restart claude-task-watcher.service
...
日志位于 `~/.claude/logs/`，超过 10MB 自动轮转，保留最近 5 个备份（`.1` ~ `.5`）。
...
通过任务 frontmatter 的 `assignee` 字段指定执行 agent：

| assignee | Agent | 适用场景 |
|----------|-------|---------|
| `codex` | Codex CLI | 默认，后台非交互模式 |
| `claude` | Claude Code | 原生 Skill 机制 |
| `claude+human` | Claude Code + 人工 review | 需要人工参与 gate |

daemon 会按 `assignee` 字段为每个任务选择对应 agent 执行。未设置时回退到 `TASK_RUNNER_AGENT` 环境变量（默认 `codex`）。

> ⛔ `switch_settings` 已弃用。旧字段仅在 `assignee: claude` 路径下仍有残留效果（通过 `claude-gateway.sh` wrapper 切换 API endpoint）。
