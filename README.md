...
| `use_aigateway` | bool | 动态切换 `~/.claude/settings.json` 为 `settings_aigateway.json`，使用备选模型（如 GPT-5.5） |
...
如果 DeepSeek 模型因为网络或其他原因不可用，可以在任务 frontmatter 中设置 `use_aigateway: true`。daemon 会在调用 `claude -p` 之前自动将 `~/.claude/settings.json` 替换为 `~/.claude/settings_aigateway.json`（指向备选模型，如 GPT-5.5），任务完成后自动恢复原始配置。
