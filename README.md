...
如果 DeepSeek 模型因为网络或其他原因不可用，可以在任务 frontmatter 中设置 `switch_settings: true`。daemon 会使用 `~/.claude/claude-gateway.sh` wrapper 脚本代替 `claude` 执行任务——wrapper 动态注入 API key 和 base URL，通过 `--settings` 指定 `settings_aigateway.json`（只含模型/插件/hooks 配置，不含 key），不污染原始 `settings.json`。

前提条件：
1. `~/.claude/claude-gateway.sh` 存在且可执行
2. `~/.claude/settings_aigateway.json` 存在且配置正确（模型、插件等）
3. wrapper 脚本中 `get-api-key.sh` 能正常获取有效的 API key
