...
自动创建 TASK-xxx.md ──(填写 project + assignee)──> ready
        │
        ▼
Grilling 对齐 ──(Kitty tab 通知)──> needs-grilling
        │                               │ 你完成交互式 grilling 对话
        ▼                               ▼
...
| `ready` | 可以开始 | 等待 daemon 触发 Grilling 通知 |
| `needs-grilling` | 等待你交互式对话对齐需求或解决阻塞 | 在 Kitty 新 tab 中与 OMP 对话，完成后 status 自动恢复 |
| `plan-review` | 计划已生成 | 审阅计划 + ADR 提议，确认后设 `plan_approved: true`（和可选的 `adr_approved: true`） |
