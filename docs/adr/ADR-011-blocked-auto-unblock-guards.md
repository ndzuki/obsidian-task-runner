...
v0.18.4 排查发现 `processBatchSequential` 将 phase-failure blocked 任务（`blocked_phase` 非空、`resume_approved=false`）与正常依赖阻塞任务同等处理，导致每次扫描都 auto-unblock → 重新调度失败 → 再 blocked → 死循环。同时 OMP 被 kill 后 PID gate 无 cooldown，watcher 触发即重调，产生调度风暴。
...
2. **Defensive `blocked_phase` 补全**：`blocked` + `blocked_phase=""` + 有 phase error → 自动补 `"implementing"`（防御 OMP skill 的畸形写入）。
3. **失败 cooldown**：`handlePhaseFailure` 写 blocked 后记录 `phaseFailures[taskPath] = time.Now()`。下次扫描若 `time.Since < 2min` → 跳过该任务。
4. **SIGTERM 不 fallback**：OMP exit code 为负（信号终止）→ 跳过 fallback 模型重试，直接 blocked（省 token + 时间）。
...
- 消除 OMP 被杀后的重复调度风暴（027 17 次/064 6 次）
