...
| planning | `max` | 深度思维链计划（自 high 上调，见 Updates） |
...
- **复盘后上调**：priority/refining 从 `off`/`low` 上调 `medium`——低强度下 spec 命名推断类失误（字段名与门禁 fixture 不一致）证明 low 不够，但 high 对每轮 refining 太贵。部分执行路径不声明 off 档位（`reasoningEffort:"off"` → UNSUPPORTED），medium 同时解决档位可用性问题。
- **planning 深度上调**：planning 从 `high` 上调 `max`（执行后端最高推理档）——plan 是全任务最高杠杆产物，被每个 AC 迭代消费；plan-review 人审拦方向性错误、拦不住字段契约类细节，plan 缺陷在 round2 逐 AC 引爆；2-3× token 只付一次（planning 是稀有阶段），性价比最高。配套 `phase_timeouts_minutes.planning` 30→45。
- **会话后端统一（审计后）**：design 会话从 spawn 适配器迁入 embed 执行后端（`newDesignExecutor` 与 phaseExecutor 同后端选择）。spawn 路径不传 reasoningEffort，此前 design=max 从未生效、强度实际落在 `settings.yaml` `agent-default-model` 的 profile 默认（当时恰为 xhigh，纯属巧合）；迁移后 design=max 真实生效，同时获得 durable resume 与 fallback 下发。
