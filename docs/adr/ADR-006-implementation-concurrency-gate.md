...
`max_concurrent_tasks` 需要限制同时执行 Round 2（implementing）的 OMP 实例数，但 refining/planning/priority/merge 阶段不限。实现需要处理 daemon 重启后仍在运行的遗留 Round 2 进程。

## Decision

**`implementationGate`：本地槽 + PID 采纳双机制**。

1. **本地槽**（`tryAcquireLocal`/`releaseLocal`）：当前 daemon 实例启动的 OMP，通过计数信号量控制并发。
