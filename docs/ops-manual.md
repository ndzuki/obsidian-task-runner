...
- 环境收尾（k3d 临时资源）
- 并发控制（阶段并发 / 计划文件重叠 / 全局封顶）
- 阻断任务自动恢复
- 知识库（语义检索 / 交互会话 KB-first / 精排 / 问答 / 会话结束提炼）
- 推理强度路由
- 依赖自动恢复与 API Key
- 优雅停机
- 故障排查
...
```mermaid
flowchart LR
    FAIL[阶段失败] -->|首次错误码+阶段| SINK[自动沉淀模式: 现象/根因/修复/经验]
...
- **自动沉淀**：阶段失败时，`handlePhaseFailure` 自动把首次出现的错误码+阶段组合（`API_KEY_UNAVAILABLE`/`PHASE_INTERRUPTED`/`MODEL_FAILED` 等）追加为知识库「模式」（现象→根因→修复→经验），按错误码+阶段去重，跨重启有效——**踩坑在发生时即记录，不等人工**。
