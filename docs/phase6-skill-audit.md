...
> 描述质量、omp 残留与结构化输出契约。参考 `skill://config-reviewer` 与
> `skill://writing-for-agents` 标准。

## 1. 扫描概况

- 共 **37** 个 skill（35 个 omp 迁移 + `dsh-upgrade` + `obsidian-task-runner-design`）。
...
| # | skill | 问题 | 影响 |
|---|---|---|---|
| 1 | **omp-tools** | 整个 skill 是 oh-my-pi 工具集：正文引用 DSH 不存在的工具 `recipe`/`browser`/`lsp`/`ast_edit`/`rename_file`/`debug`，且引用 omp 配置路径 `~/.omp/agent/`、`omp --version`、版本敏感表（17.2.x） | `dm=0`（模型可加载）→ 模型加载后**尝试调用不存在的工具**，执行失败；描述的是已弃用的 omp 工具链 |

### 🟡 警告（建议修复）

| # | skill | 问题 | 影响 |
|---|---|---|---|
| 2 | obsidian-task-runner（主） | 正文 8+9 处 `OMP` 机制描述（"OMP 会话写回"、"fallback_models 重启 OMP"、"OMP exit"、"20+ OMP 会话"）——daemon 已默认切 dsh（5.9） | 文档过时，非执行错误；误导后续维护 |
| 3 | obsidian-task-runner-round2 / merge | 各 2-4 处 OMP 描述 | 同上 |
| 4 | knowledge-base / kulala-http / writing-for-agents / config-reviewer | 1-10 处 `omp` 引用，多为示例/历史描述 | 轻微 |
...
| 8 | 单一事实源 | skills manifest + install.go + sync-omp-skills 已一致（design 已入 manifest，37 个） |
| 9 | 触发词质量 | 多数 description 前置核心词 + 明确触发词，符合 writing-for-agents；个别（pm/rebaseline）过长 |

## 3. 优化方案（按优先级）

### P0 — omp-tools 处理（唯一执行风险）

**选项 A（推荐）：删除** —— `~/.dsh/AGENTS.md` 已内置 omp→DSH 工具映射表
（recipe→make/just、browser→web_search、lsp/ast_edit/rename_file→grep/read+edit/write、
debug→bash 复现），omp-tools 冗余且描述不存在的东西。删除后模型不会再被误导
调用 recipe/browser/lsp 等工具。

**选项 B：改造** —— 改为「DSH 工具纪律」精简版（只留读/写两档纪律 + 冒烟清单），
删除所有 omp 工具引用 + 版本敏感表 + omp 配置路径。

### P1 — obsidian-task-runner 系列 omp 描述更新

把主 skill + round2 + merge 里的 `OMP` 机制描述改为 `dsh headless`：
- "OMP 会话写回" → "执行会话写回"
- "fallback_models 重启 OMP" → "fallback 链（DSH fallback.mjs）"
- "OMP exit" → "执行器退出"
- 删除 omp config.yml / watchEmptyStops（已废弃）相关描述
...
1. **P0**：删除或改造 omp-tools（消除唯一执行风险）。
2. **P1**：obsidian-task-runner 系列 OMP 描述更新（文档对齐 dsh 现实）。
3. **P2**：两个过长 description 精简。
4. **P3**：随 embed 迁移解决 strict JSON 契约。

> 注：omp-tools 的删除/改造涉及用户工作流，需确认后再执行（本报告仅提议）。
