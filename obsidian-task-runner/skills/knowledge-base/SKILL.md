...
**触发**：
- **自动**：OMP 扩展 `.omp/extensions/kb-session-distill.ts`（复制到 `~/.omp/agent/extensions/` 用户级全项目生效，或留在 `<repo>/.omp/extensions/` 项目级自动发现；禁用：config.yml `disabledExtensions`）监听 `session_stop`（主会话停止钩子，task/subagent 会话不触发），满足条件（会话有实质工作 + 达到长度阈值）时 `continue` 并注入提炼指令。**每个有实质工作的主会话都触发一次**（不再做「同日一次」的跨会话去重——多轮 `/new` 会话各自沉淀，重复内容由 `otg kb absorb` 内置归一化去重兜底）。
