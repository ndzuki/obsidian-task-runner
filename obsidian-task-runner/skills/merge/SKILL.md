...
## Environment Cleanup（环境清理，强制）

Merge 会话退出前（成功或失败）必须：

1. 删除本任务在本地创建/遗留的一切临时资源：k3d 集群、docker 容器/网络、临时凭据、冒烟日志与构建产物；优先调用项目清理目标（`make dev-down`/`dev-purge`/`k3d-clean`）。
2. 严禁停止/删除用户常驻服务（kb-reranker、ollama-sycl、桌面进程等）或其它任务资源；不以停用户服务换取门禁。
3. 保留给下游任务的环境必须在任务文档显式声明下游任务 ID 与保留清单，由下游任务结束时清理。
4. 清理结果与 `k3d cluster list`/`docker ps` 快照写入阶段记录；清理失败列出残留清单如实上报。
