...
refining 会话若创建过任何临时资源（原型、容器、集群、临时文件），退出前必须清理并记录证据；严禁停止/删除用户常驻服务（本地推理/向量检索常驻服务等）或其它任务资源换取门禁。
...
- **成熟度**：{fully_mature / mostly_mature / immature}——{一句话理由}。
- **路由**：{planning / grilling / park}——{一句话依据}。
- **ADR 一致性**：{通过 / 冲突项}——冲突时逐条列「ADR-xxx 约束 vs REQ 违反点」。
- **相对上次 refine 的增量**：{cosmetic 注解 / 实质变更}——{一句话}。

**知识重关联**（`- ` 分点，无新增则单行「无新增领域术语」）：

- **新增术语**：{术语列表，空则「无」}。
- **知识命中**：{文档路径 + 一句话应用提示，空则「无」}。
- **知识缺口**：{缺口主题，供后续入库，空则「无」}。
```

**Markdown 写回铁律**（本 Skill 及所有 phase skill 通用）：写回 Vault 文档（REQ/TASK/清单/评估）必须用结构化 Markdown——`##/###` 标题分层、`| 表格 |` 对齐数据、`- ` 分点拆长内容、`> ` 引用块标注来源/裁决、``` 代码块包裹 YAML/JSON/代码。**禁止把结论/评估/说明写成一段超长纯文本**（不分点、不换行）——读者一眼扫不到重点即视为写回失败，重写。
