...
return {
  ...
  s('otask', {
    ...
    t({ '', '# 🟡 推荐', 'tags: []', 'epic: ""', 'blocked_by: []', 'blocks: []', 'target_env: staging', 'stage: ""', 'stage_source: ""   # req=REQ 继承（跟随 REQ 变更）；空=daemon 分组/PM 手动（不跟随）', 'plan_files: []     # Round 1 计划将修改的文件清单（repo 相对路径），daemon 用于并发重叠预警', 'new_project: false' }),
