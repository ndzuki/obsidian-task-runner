...
22. **环境清理是硬性契约（Environment Cleanup）**：任何阶段会话退出前必须清理本任务创建的一切临时资源（k3d 集群、docker 容器/网络、临时凭据、日志产物），优先调用项目清理目标（`make dev-down`/`dev-purge`/`k3d-clean`），清理证据写入阶段记录；**严禁停止/删除用户常驻服务**（本地推理/向量检索常驻服务、桌面进程等）或其它任务资源换取门禁通过。资源保留给下游必须显式声明下游任务 ID 与保留清单，由下游任务结束时清理。
